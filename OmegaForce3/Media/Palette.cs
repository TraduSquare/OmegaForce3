...
namespace Ninoimager.Format
{
	public class Palette
	{
		...
		public void ToWinPaletteFormat(string outPath, int index, bool gimpCompability)
		{
			...
			{
				bw.Write((byte)palette[index][i].R);
				bw.Write((byte)palette[index][i].G);
				bw.Write((byte)palette[index][i].B);
				bw.Write((byte)0x00);
				bw.Flush();
			}
		...
		public void ToAcoFormat(string outPath, int index)
		{
			...
			for (int i = 0; i < palette[index].Length; i++) {
				...
				bw.Write(ToBytesBE((ushort)palette[index][i].R));     // Red component
				bw.Write(ToBytesBE((ushort)palette[index][i].G));   // Green component
				bw.Write(ToBytesBE((ushort)palette[index][i].B));    // Blue component
